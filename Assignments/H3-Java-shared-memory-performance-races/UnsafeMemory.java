class UnsafeMemory {
  ...
  private static int parseInt(String s, int min, int max) {
    ...
    if (n < min || n > max) throw new NumberFormatException(s);
    return n;
  }
  ...
  private static void test(byte[] input, byte[] output, byte maxval) {
    ...
    for (int i = 0; i < input.length; i++) {
      isum += input[i];
      osum += output[i];
      if (output[i] < 0) error("negative output", output[i], 0);
      if (output[i] > maxval) error("output too large", output[i], maxval);
    }
    if (isum != osum) error("sum mismatch", isum, osum);
  }
